...
batchSize = None # seems like the code does not support minibatch right now

# Initialize network
net = HebbFeatureLayer(init=[d, Nh, Ny], Nx=Nx)
# Load pretrained weights
pretrained_weights = "publish/conv/HebbNet[50,16,1]_train=cur1_incr=plus1_w1init=randn_b1init=scalar_w2init=scalar.pkl"
net.load(pretrained_weights)
print("Eta:", net.eta, "Lambda:", net.lam) # Check that the parameters are loaded correctly
print("Pretrained weights loaded successfully.")

# Freeze the Hebbian weights
net.w1.requires_grad_(False)
# net.b1.requires_grad_(False)
# net.w2.requires_grad_(False)
# net.b2.requires_grad_(False)
# net.lam.requires_grad_(False)
# net.eta.requires_grad_(False)

# Only the featurizer will be trained
...
net.fit('curriculum', gen_data, iters=float('inf'), itersToQuit=(2*10**6), batchSize=batchSize, learningRate=1e-3,
        filename=save_weights_file, overwrite=False, folder=logs, R0=1, Rf=Rf, increment=increment)
...
ax.figure.save_figure("results/gen_plot_feature_only.png")